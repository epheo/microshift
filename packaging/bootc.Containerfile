...
# The centos-bootc base stopped shipping kernel-modules-extra, and OVN-K
# hard-fails without xt_CT/nft_compat (ovnkube-node's geneve NOTRACK rule):
# the node never goes Ready. When the CS10 repo has already moved past the
# base's kernel, dnf upgrades the whole kernel set in place (initramfs is
# regenerated by the kernel scriptlets); the one-kernel guard below still
# applies.
RUN dnf -y install kernel-modules-extra && dnf clean all
